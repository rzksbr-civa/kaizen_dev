...
class Model_client_inventory_optimization_board extends CI_Model {
	...
	public function get_client_inventory_optimization_board_data($data) {
		...
		$local_created_time_field = "IF(sales_flat_order_stock.stock_id=3,CONVERT_TZ(sales_flat_order.created_at,'UTC', 'US/Mountain'),CONVERT_TZ(sales_flat_order.created_at,'UTC','US/Eastern'))";
		...
		$data['client_inventory_optimization_board_2d_data'] = array();
		foreach($sku_data as $sku => $current_data) {
		...
		$data['client_inventory_optimization_board_visualization_html'] = $this->load->view(PROJECT_CODE.'/view_client_inventory_optimization_board_visualization', $data, true);
