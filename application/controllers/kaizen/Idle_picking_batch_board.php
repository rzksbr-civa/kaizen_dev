...
class Idle_picking_batch_board extends CHCHDB_Controller {
	...
	public function index() {
		$header_data = array();
		$header_data['page_title'] = generate_page_title('Idle Picking Batch Board');
		...
		if($data['generate']) {
			$data = $this->model_idle_status->get_idle_picking_batch_board_data($data);
		}
		
		$footer_data['js'] = $this->load->view(PROJECT_CODE.'/js_view_idle_picking_batch_board', $data, true);

		$this->load->view('view_header', $header_data);
		$this->load->view(PROJECT_CODE.'/view_idle_picking_batch_board', $data);
