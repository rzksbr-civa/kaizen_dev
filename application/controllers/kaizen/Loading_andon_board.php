...
class Loading_andon_board extends CHCHDB_Controller {
	...
	public function index() {
		$header_data = array();
		$header_data['page_title'] = generate_page_title('Loading Andon Board');
		...
		if($data['generate']) {
			$data = $this->model_loading_andon->get_loading_andon_board_data($data);
		}
		
		$footer_data['js'] = $this->load->view(PROJECT_CODE.'/js_view_loading_andon_board', $data, true);

		$this->load->view('view_header', $header_data);
		$this->load->view(PROJECT_CODE.'/view_loading_andon_board', $data);
