...
class Inventory_board extends CHCHDB_Controller {
	...
	public function index() {
		$header_data = array();
		$header_data['page_title'] = generate_page_title('Inventory Board');
		...
		if($data['generate']) {
			$data = $this->model_inventory->get_inventory_board_data($data);
		}
		
		$footer_data['js'] = $this->load->view(PROJECT_CODE.'/js_view_inventory_board', $data, true);

		$this->load->view('view_header', $header_data);
		$this->load->view(PROJECT_CODE.'/view_inventory_board', $data);
