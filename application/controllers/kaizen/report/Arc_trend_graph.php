...
class Arc_trend_graph extends CHCHDB_Controller {
	...
	public function index() {
		...
		$header_data = array();
		$header_data['page_title'] = generate_page_title('ARC Trend');
		...
		$data['num_days'] = (strtotime($data['period_to']) - strtotime($data['period_from'])) / 86400;
		...
		if($generating_graph) {
			...
			$data['graph'] = $this->model_kaizen->get_trend_graph_data($args);
		}
		
		$footer_data['js'] = $this->load->view(PROJECT_CODE.'/report/js_view_arc_trend_graph', $data, true);

		$this->load->view('view_header', $header_data);
		$this->load->view(PROJECT_CODE.'/report/view_arc_trend_graph', $data);
