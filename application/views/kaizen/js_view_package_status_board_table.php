...
<script>
	...
	function update_status(carrier_code, track_number, mwe_status, mwe_expected_delivery_date, row_index) {
		$.post('<?php echo base_url(PROJECT_CODE.'/api/get_carrier_tracking_status'); ?>', { 
			...
			if(result.success) {
				...
				table.cell({row: row_index, column: 6}).data(result.status);
				table.cell({row: row_index, column: 8}).data(result.actual_delivery_date);
				table.cell({row: row_index, column: 9}).data(result.is_delivered);
				table.cell({row: row_index, column: 11}).data(result.last_checked_at);
		...
		$.post('<?php echo base_url(PROJECT_CODE.'/api/bulk_update_carrier_tracking_status'); ?>', { 
			...
			if(result.success) {			
				for(var i=0; i<result.updated_packages.length; i++) {
					...
					table.cell({row: row_index, column: 6}).data(updated_package.status);
					if(updated_package.actual_delivery_date != null) {
						table.cell({row: row_index, column: 8}).data(updated_package.actual_delivery_date.substr(0,10));
					}
					if(updated_package.is_delivered) {
						table.cell({row: row_index, column: 9}).data('Yes');
					}
					table.cell({row: row_index, column: 10}).data(updated_package.late_days);
					table.cell({row: row_index, column: 11}).data(updated_package.last_checked_at);
	...
	<?php if($report_type == 'zone_and_state'): ?>
		var table = $('#carrier-zone-summary-table').DataTable( {
			dom: 'Z<"col-md-6"l><"col-md-6"f>r<"table-container"t><"col-md-12"i><"col-md-6"B><"col-md-6"p>',
			buttons: [
				{
					extend: 'excel',
		...
		var table = $('#carrier-state-summary-table').DataTable( {
			dom: 'Z<"col-md-6"l><"col-md-6"f>r<"table-container"t><"col-md-12"i><"col-md-6"B><"col-md-6"p>',
			buttons: [
				{
					extend: 'excel',
