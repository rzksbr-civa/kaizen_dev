...
<style type="text/css">
	...
	#client-inventory-replenishment-board-table_wrapper .download-btn-group {
		text-align: right;
	}
</style>
...
<script>
	...
	var client_inventory_replenishment_board_table = $('.client-inventory-replenishment-board-table').DataTable( {
		...
		data: <?php echo json_encode($client_inventory_replenishment_board_2d_data); ?>,
		deferRender: true,
		columnDefs: [
	...
	function updateHistoricalInventoryLevelsGraph(sku) {
		...
		$.post(
			...
			function(result) {
				...
				console.log('max:'+result.projected_stock_out_graph_data_max);
				console.log('min:'+result.projected_stock_out_graph_data_min);
			},
			"json" 
		);
	}
